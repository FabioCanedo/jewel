...
C--medium parameters
     ...
     & TPROFILE(243,243,200),NF
      INTEGER NF
      DOUBLE PRECISION CENTRMIN,CENTRMAX,BREAL,CENTR,RAU,
     & TPROFILE
      COMMON/MEDFILEC/MEDFILE,FILELIST,NLIST
      CHARACTER*80 MEDFILE,FILELIST
      INTEGER NLIST
	  ...
	  do 20 i=1,1000
	     ...
	     if (firstchar.eq.'#') goto 20
          ...
          ELSE IF (LABEL=="FILELIST") THEN
            READ(BUFFER,*,IOSTAT=IOS) FILELIST
...
C--local variables
      ...
      ZVAL=PYR(0)*MEDPART(XVAL,YVAL,0.0d0,0.0d0)
      
      X1=BREAL/2.-RAU
      X2=RAU-BREAL/2.
      Y1=-SQRT(4*RAU**2-BREAL**2)/2.
      Y2=SQRT(4*RAU**2-BREAL**2)/2.
 132  XVAL=PYR(0)*(X2-X1)+X1
      YVAL=PYR(0)*(Y2-Y1)+Y1
      ZVAL=PYR(0)*NTHICK(-BREAL/2.,0d0)*NTHICK(BREAL/2.,0d0)
      Z=MEDPART(XVAL,YVAL,0.0d0,0.0d0)
      IF(ZVAL.GT.Z) GOTO 132
...
C--local variables
     ...
     &NTHICK,ys,MEDPART,STEP,INTERPOLATE
      DATA PI/3.141592653589793d0/
      ...
      STEP=20.0/(243-1)
      GETTEMP=INTERPOLATE(TPROFILE,X4,Y4,TAU,10.0d00,-10.0d00,STEP)
...
C--local variables
 ...
 30   CONTINUE
     ...
     &SIGMANN
      ...
      DOUBLE PRECISION CENTRMIN,CENTRMAX,BREAL,CENTR,RAU,
     & TPROFILE 
      CHARACTER*2200 BUFFER,LABEL*5
      CHARACTER DUMMIE
      DOUBLE PRECISION Y(243,243)
      DOUBLE PRECISION X(243)
      INTEGER I,K,SIGNAL,POS

      IF(MEDFILELIST) THEN
      OPEN(UNIT=75,FILE=FILELIST)
      DO I=1,NLIST-1
      READ(75,*)
      ENDDO
      READ(75,'(A)') MEDFILE
...
163   READ(10,'(A2200)',IOSTAT=SIGNAL) BUFFER
      IF(SIGNAL>0) THEN
      WRITE(*,*) "Something went wrong with signal: ", SIGNAL
      STOP
      ELSE IF(SIGNAL<0) THEN
...
113   FORMAT(100ES8.2)
      ...
      BUFFER=""
          DO 20 I = 2,243
            READ(10,'(A2200)',IOSTAT=SIGNAL) BUFFER
            READ(BUFFER,*,IOSTAT=SIGNAL) X
...
164   CLOSE(10)
     ...
     & TPROFILE
      ...
      XMAX=10.0
      XMIN=-10.0
      STEP=(XMAX-XMIN)/(243-1)
C     TAU=SQRT(T4**2-Z4)
      TAU=TAUI
      MEDPART=INTERPOLATE(TPROFILE,X4,Y4,TAU,XMAX,XMIN,STEP)
      END

      DOUBLE PRECISION FUNCTION INTERPOLATE(TEMP,X4,Y4,T4,XMAX,
     &XMIN,STEP)
      IMPLICIT NONE
      DOUBLE PRECISION TEMP(243,243,200)
      DOUBLE PRECISION X4,Y4,XMAX,XMIN,T4
      INTEGER I,J,N,K
      ...
      ITEMP=0.0
      FTEMP=0.0
      N=INT(MAX((XMAX-XMIN+STEP)/STEP,0.0))
      ...
      IF(I>243.OR.J>243.OR.K>200.OR.I<0.OR.J<0.OR.K<0) THEN
      INTERPOLATE=0.0
      RETURN
      ENDIF

      ITEMP=(1.0-T)*(1.0-U)*TEMP(I,J,K)+(T)*(1.0-U)*TEMP(I+1,J,K)
     &+(1.0-T)*(U)*TEMP(I,J+1,K)+(T)*(U)*TEMP(I+1,J+1,K)


      FTEMP=(1-T)*(1-U)*TEMP(I,J,K+1)+(T)*(1-U)*TEMP(I+1,J,K+1)
     &+(1-T)*(U)*TEMP(I,J+1,K+1)+(T)*(U)*TEMP(I+1,J+1,K+1)
    
      INTERPOLATE=ITEMP+(FTEMP-ITEMP)*FRA

      IF(INTERPOLATE.GT.1) THEN
      WRITE(*,*) "Interpolate: ",FTEMP, ITEMP
      ENDIF
      END
