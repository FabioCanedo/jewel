...
C--local variables
	...
	character*2 b1,b2
      ...
      COMMON/MEDPARAMS/etamax,intmass,FILEMED
      
      CHARACTER*80 FILEMED
      DOUBLE PRECISION etamax,intmass
      
      integer nsteps

      character*80 mymedname
      
      DOUBLE PRECISION TAUI
      
      DOUBLE PRECISION X,Y,Z,T
      
      DOUBLE PRECISION XMAX,XMIN,MYSTEP
      
      DOUBLE PRECISION MYMEDPROF(243)
      ...
      nsteps=243
      mystep=(xmax-xmin)/(nsteps-1)

      logfid=3
      FILEMED = 'medparams/modmed.1.dat'
      etamax = 3.1
      mass = 208.d0
...
C--initialize medium
      intmass = int(mass)
C     CALL MEDINIT(FILEMED,logfid,etamax,intmass)
      CALL MEDNEXTEVT
      call getarg(2,mymedname)
      write(*,*) mymedname

      OPEN(1,FILE=mymedname,STATUS='UNKNOWN')
...
30    CONTINUE
      ...
      write(*,*) "Testing ",gettemp(0.0d0,0.0d0,0.0d0,0.6d0)
      !stop 
      DO KK=1,200
      ...
      MYMEDPROF(J)=GETTEMP(X,Y,Z,T)
      !if(gettemp(x,y,z,t).gt.0.15) then
      !mass = gettemp(x,y,z,t)
      !write(*,*) x,y,t,z,mass
      !endif
